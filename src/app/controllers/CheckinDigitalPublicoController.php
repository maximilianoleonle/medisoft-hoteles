<?php
/**
 * Check-in digital publico (bloque checkin_digital, sin login).
 ...
 * del motor. Rate limit reutiliza login_intentos.
 */

require_once __DIR__ . '/../services/CheckinDigitalService.php';

class CheckinDigitalPublicoController extends Controller {

    public function formularioAction($slug, $token) {
        [$hotel, $link] = $this->resolver($slug, $token);

        View::renderTemplate('checkin/formulario', [
            'title' => 'Pre-registro - ' . ($hotel['nombre'] ?? 'Hotel'),
            'hotel' => $hotel,
    ...
    public function completarAction($slug, $token) {
        if (!$this->isPost()) {
            $this->redirect('h/' . $slug . '/checkin/' . $token);
        }

        [$hotel, $link] = $this->resolver($slug, $token);

        if (!$this->permitirSolicitud((string) $hotel['slug'])) {
            $resultado = ['success' => false, 'message' => 'Demasiados intentos. Espera un minuto e intenta de nuevo.'];
        } else {
            $servicio = new CheckinDigitalService();
            $resultado = $servicio->completar((int) $hotel['id'], (string) $token, $_POST, $_FILES['identificacion'] ?? null);
        }

        // Releer el link para reflejar el estado final en la vista.
        $servicio = $servicio ?? new CheckinDigitalService();
        $link = $servicio->porToken((int) $hotel['id'], (string) $token);
    ...
    /** Resuelve hotel + link o corta con pagina amable. */
    private function resolver($slug, $token) {
        ...
        if (preg_match('/^[a-z0-9](?:[a-z0-9-]{0,118}[a-z0-9])?$/', $slug)) {
            try {
                ...
                error_log('Checkin publico: error al resolver hotel: ' . $e->getMessage());
            }
        }
        ...
        if (!function_exists('hotel_has_module') || !hotel_has_module('checkin_digital', (int) $hotel['id'])) {
            $this->paginaPublica(404, 'Pre-registro no disponible', 'Este hotel no tiene habilitado el pre-registro en linea.', $hotel);
        }
        ...
        if (!$link) {
            $this->paginaPublica(404, 'Liga no valida', 'Esta liga de pre-registro no existe. Pide una nueva al hotel.', $hotel);
        }
